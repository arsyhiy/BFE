...
# ========== НОВЫЕ ПЕРЕМЕННЫЕ ДЛЯ ЛАНДШАФТА (ОТКЛЮЧЕНЫ) ==========
@export_group("Terrain following")
@export var terrain_follow_enabled: bool = false
@export var terrain_raycast_distance: float = 20.0
@export var spawn_offset: float = 0.0
@export var ground_collision_mask: int = 1
@export var force_snap_on_physics: bool = false

var terrain_ray: RayCast3D
var is_initialized: bool = false
var snap_counter: int = 0
var terrain_node: Node = null
var ground_height: float = -INF
...
func _ready() -> void:
	...
	# ===== ВСЯ СИСТЕМА ПРИВЯЗКИ ОТКЛЮЧЕНА =====
	# Ландшафт сам позаботится о позиции игрока
	# setup_terrain_ray()
	# await get_tree().process_frame
	# await get_tree().process_frame
	# find_terrain()
	# force_snap_to_terrain()
	# ===========================================
	
	# Отметка инициализации
	is_initialized = true
	
	print("✅ Игрок инициализирован! (Привязка к ландшафту отключена)")

# ========== ПОИСК ЛАНДШАФТА (ОТКЛЮЧЕН) ==========
func find_terrain() -> void:
	pass  # Функция отключена

# ========== ПРОВЕРКА КРИВЫХ ==========
func check_curves() -> void:
	if desired_move_speed_curve == null:
		desired_move_speed_curve = Curve.new()
		desired_move_speed_curve.add_point(Vector2(0, 0))
		desired_move_speed_curve.add_point(Vector2(0.5, 0.5))
		desired_move_speed_curve.add_point(Vector2(1, 1))
		print("⚠️ Создана кривая desired_move_speed_curve по умолчанию")
	
	if in_air_move_speed_curve == null:
		in_air_move_speed_curve = Curve.new()
		in_air_move_speed_curve.add_point(Vector2(0, 0))
		in_air_move_speed_curve.add_point(Vector2(1, 1))
		print("⚠️ Создана кривая in_air_move_speed_curve по умолчанию")

# ========== СОЗДАНИЕ ЛУЧА (ОТКЛЮЧЕН) ==========
func setup_terrain_ray() -> void:
	pass  # Функция отключена

# ========== ПРИНУДИТЕЛЬНАЯ ПРИВЯЗКА (ОТКЛЮЧЕНА) ==========
func force_snap_to_terrain() -> void:
	pass  # Функция отключена

# ========== ПОЛУЧЕНИЕ ВЫСОТЫ ЗЕМЛИ (ОТКЛЮЧЕНА) ==========
...
func _physics_process(delta: float) -> void:
	...
	# Применяем прилипание к земле (ОТКЛЮЧЕНО)
	# if terrain_follow_enabled:
	# 	apply_terrain_follow(delta)
	
	move_and_slide()
	
	# Аварийная проверка (ОТКЛЮЧЕНА)
	# if Engine.get_process_frames() % 100 == 0:
	# 	if global_position.y < -5.0:
	# 		print("🚨 Персонаж провалился! Принудительная привязка...")
	# 		force_snap_to_terrain()
	# 	
	# 	if not is_on_floor() and velocity.y < 0 and global_position.y < 0:
	# 		print("⚠️ Восстановление позиции...")
	# 		force_snap_to_terrain()
...
func tween_hitbox_height(state_hitbox_height: float) -> void:
	...
	if hitbox != null:
		hitbox_tween.tween_method(
			func(value): set_hitbox_height(value), 
			hitbox.shape.height, 
			state_hitbox_height, 
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_F1:
			...
			print("  На полу: ", is_on_floor())
			print("  Скорость: ", velocity)
			print("  ⚠️ Система привязки ОТКЛЮЧЕНА!")
		
		if event.keycode == KEY_F2:
			print("🔄 Принудительная привязка (F2) - ОТКЛЮЧЕНА!")
			print("  Используйте F5 в ландшафте для привязки")
		
		if event.keycode == KEY_F3:
			print("🚀 Телепорт на 10 метров вверх")
